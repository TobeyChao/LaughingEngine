...
void DynamicDescriptorHeap::CleanupUsedHeaps(uint64_t Fence)
{
	RetireCurrentHeap();
...
D3D12_GPU_DESCRIPTOR_HANDLE DynamicDescriptorHeap::UploadDirect(D3D12_CPU_DESCRIPTOR_HANDLE Handle)
{
	...
	Graphics::g_Device->CopyDescriptorsSimple(1, DestHandle, Handle, m_DescriptorType);
...
ID3D12DescriptorHeap* DynamicDescriptorHeap::RequestDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE HeapType)
{
	...
	while (!sm_RetiredDescriptorHeaps[idx].empty() && Graphics::g_CommandManager.IsFenceComplete(sm_RetiredDescriptorHeaps[idx].front().first))
	{
...
{
	// Root parameter is a CBV_SRV_UAV descriptor table
